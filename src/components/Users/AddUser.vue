...
function useAdd(addFormRef, emit) {
  const submitAdd = () => {
    addFormRef.value.validate((vaild) => {
      if (vaild) {
        // addFormRef.value.model:表单绑定的数据对象   :model="addForm"  直接当做params参数
        console.log(addFormRef.value.model);
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const addFormRules = {
      ...
      userMail: [
        // validator:自定义校验规则  后面的就是el中的自定义的校验函数
